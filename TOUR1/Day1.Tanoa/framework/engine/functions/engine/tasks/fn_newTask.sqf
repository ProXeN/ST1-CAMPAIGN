...
if (isNil "_parent") then {
  _newTask = [_side, _id, _ndetails, nil, false, nil, true, _type, true] call BIS_fnc_taskCreate;
} else {
  _newTask = [_side, [_id, _parent], _ndetails, nil, false, nil, true, _type, true] call BIS_fnc_taskCreate;
};
//systemchat str(_newTask);
_newTask = [_id] call BIS_fnc_taskReal;
